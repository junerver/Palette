 ...
 *
 * 本模块 Compose-free：选区类型使用 [MarkdownSelection]，
 * 返回类型使用同包下 [MarkdownEditResult]（定义在 MarkdownModels.kt）。
 */
// region 辅助：行范围
...
public fun selectedLineRanges(text: String, selection: MarkdownSelection): List<Pair<Int, Int>> {
    val (firstStart, _) = currentLineRange(text, MarkdownSelection(selection.min, selection.min))
...
public fun wrapSelection(
    ...
    return if (hasSelection) {
        ...
        if (inner.startsWith(prefix) && inner.endsWith(suffix) && inner.length >= prefix.length + suffix.length) {
            ...
            MarkdownEditResult(newText, MarkdownSelection(start, start + unwrapped.length))
        } else {
            val newText = text.substring(0, start) + prefix + inner + suffix + text.substring(end)
            MarkdownEditResult(newText, MarkdownSelection(start, start + prefix.length + inner.length))
        }
    } else {
        val newText = text.substring(0, start) + prefix + suffix + text.substring(end)
        MarkdownEditResult(newText, MarkdownSelection(start + prefix.length, start + prefix.length))
    }
}
// endregion
...
public fun toggleLinePrefix(
    ...
    val newSel = if (firstNewStart < 0) selection else MarkdownSelection(firstNewStart, lastNewEnd)
...
public fun toggleTaskItem(text: String, selection: MarkdownSelection): MarkdownEditResult {
    ...
    return MarkdownEditResult(newText, MarkdownSelection(selStart, selStart))
}
// endregion
...
public fun setHeadingLevel(text: String, selection: MarkdownSelection, level: Int): MarkdownEditResult {
    ...
    sb.append(text, writeCursor, text.length)
    val newSel = if (firstStart < 0) selection else MarkdownSelection(firstStart, lastEnd)
    return MarkdownEditResult(sb.toString(), newSel)
}
...
/** 默认代码块模板。 */
public fun defaultCodeFence(language: String = ""): String =
    "```$language\n\n```\n"
// endregion
...
public fun continueOnEnter(text: String, selection: MarkdownSelection): MarkdownEditResult? {
    ...
    if (content.isBlank()) {
        ...
        return MarkdownEditResult(newText, MarkdownSelection(lineStart, lineStart))
    }
    ...
    return MarkdownEditResult(newText, MarkdownSelection(newCursor, newCursor))
}
// endregion
