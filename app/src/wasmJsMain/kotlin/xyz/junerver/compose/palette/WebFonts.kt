 ...
 *
 * Skiko (the wasmJs canvas backend) does not fall back to the OS/browser font list the way the JVM
 * and Android backends do, so CJK glyphs render as tofu (□) unless a font covering them is loaded
 * explicitly. We bundle Noto Sans SC via Compose Resources (which handles the wasmJs fetch for us)
 * and apply it to every TextStyle.
 *
 ...
 * [PaletteMaterialTheme] inherits the ambient MaterialTheme.typography, so this is the single
