 ...
 * - 容量有上限，避免超长编辑会话内存膨胀。
 */
internal class MarkdownHistory(
    initial: TextFieldValue,
    private val capacity: Int = DEFAULT_CAPACITY,
    private val coalesceMs: Long = DEFAULT_COALESCE_MS,
    private val now: () -> Long = { defaultNow() },
) {
    private data class Entry(val value: TextFieldValue, val time: Long, val coalescing: Boolean = false)
    ...
    /** 当前值（文本 + 光标）。 */
    val current: TextFieldValue get() = present.value
     ...
     */
    fun pushTyping(next: TextFieldValue) {
     ...
     * 3. 新光标紧邻变更点（连续输入）。
     */
    private fun shouldCoalesce(prev: TextFieldValue, next: TextFieldValue, prevTime: Long): Boolean {
